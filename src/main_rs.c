...
int main(int argc, char **argv) {
  ...
  t0 = MPI_Wtime();
  int rc = blocked_repeated_squaring_apsp(input, &mpi_out, b);
  t1 = MPI_Wtime();
  if (rc != 0) {
    if (world_rank == 0)
      fprintf(stderr, "MPI Blocked Repeated Squaring APSP failed\n");
    destroy_matrix(input);
    destroy_matrix(output_file_matrix);
    if (world_rank == 0) {
  ...
  if (world_rank == 0) {
    printf("MPI Blocked Repeated Squaring APSP:   Time = %.6f s (P=%d, p=%d, b=%d)\n", t1 - t0,
           world_size, p, b);

    assert_apsp(mpi_out, output_file_matrix, "MPI Blocked Repeated Squaring APSP");
  }
